...
from LogRegpy.bound_algorithms.mosek_fair_logistic_regression import fairLogisticRegression
import time
...
class MosekObjectiveEval:
    ...
    def __call__(self, data: ProblemData, node: Node) -> Tuple[float, float]:
        ...
        x = data.X[:, attributes]
        coefs, obj = fairLogisticRegression(x, data.y)
...
if __name__ == "__main__":
    ...
    print("Successful data collection:", dataset_collector("PEOPLE"))
    #print(dataset_collector.y)
    coefs, obj = fairLogisticRegression(dataset_collector.X, dataset_collector.y)
    print(obj)
