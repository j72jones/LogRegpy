...
class MOSEKUpperBounder(UpperBounder):
    ...
    def find_obj(self, fixed_out_varbitset: int, prev_coef=None):
        coef_arr, objective = self.model.solve(Node.varbitset_to_list(fixed_out_varbitset), prev_coef)
        coefs = {i: coef_arr[i] for i in Node.varbitset_to_list(Node.universal_varbitset & ~ fixed_out_varbitset)}
        return coefs, objective
