...
for k in range(4,21,4):
    ...
    for var_selector in var_selectors.keys():
        ...
        test_logger_ins = TestLogger(f"LogRegpy/tests/test_data/test_MYOCARDIAL_MOSEK_{var_selector}_Forward Stepwise Regression No Penalty_{k}.csv")
        ...
        test_tree = Tree(
            ...
            initial_upper_bound_strategy=StepwiseRegression(problem_data, solver="MOSEK"),
            test_logger=test_logger_ins,
            lower_bounder_fixed_in_agnostic=True
            )


        print("Successful test:", test_tree.solve(timeout = 1000000000, max_iter = sum((dataset_collector.n-i) for i in range(dataset_collector.n-k)), eps=0.01))
    



# To run:
# python -m LogRegpy.tests.all_myocardial_tests
